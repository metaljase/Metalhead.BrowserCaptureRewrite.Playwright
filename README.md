...
`BrowserCaptureRewrite.Playwright` is a .NET library that implements the abstractions in [`BrowserCaptureRewrite.Abstractions`](https://github.com/metaljase/BrowserCaptureRewrite.Abstractions) using [Playwright](https://playwright.dev/dotnet/).  It can intercept HTTP requests from a web page and capture the corresponding HTTP responses in-flight by targeting specific requests and optionally rewriting those responses - including the response for the web page itself.
